...
visorContainerElement.setStyles([
  ...
  { name: 'opacity', value: '0.55' },
  { name: 'pointer-events', value: 'none' },
]);

visorHeaderElement.setStyles([{ name: 'background-color', value: 'black' }]);

visorElement.setStyles([{ name: 'height', value: `${visorSize}px` }]);
...
document.addEventListener('mousemove', (event) => {
  ...
  visorHeaderElement.setStyles([{ name: 'height', value: `${headerHeight}px` }]);
});
